...
p_array = np.array([math.comb(10, k) for k in range(10+1)])
p_array[[0, 10]] = 0
p_array[[9, 8, 7, 6]] = p_array[[9, 8, 7, 6]] * [5, 2, 2, 1.2]
...
num_masked_each_row = rng.choice(np.arange(num_variables + 1),
                                 ...
                                 replace=True,
                                 p=p_array)
random_mask = torch.zeros([MM, num_variables])
random_mask
# %%
for j in range(MM):
    random_mask[j, rng.choice(num_variables, size=num_masked_each_row[j], replace=False)] = 1
# %%
arr = random_mask.sum(1).numpy()
print(arr)
# %%
ppp = np.zeros(num_variables+1)
elements_count = collections.Counter(arr)
for key, value in elements_count.items():
    ppp[int(key)] = value/MM
    print(f"{key}: {value/MM}")
# %%
plt.plot(np.arange(11), ppp)
plt.show()
# %%
"""
