...
def train_VAEAC_model(data_train,
    ...
    # %%
    if distribution not in ['MVN', 'GH', 'Burr']:
        print("Warning: you are not using one of the default distributions\nwith known conditional distributions: "
              "MVB, GH, and Burr.", file=stderr, flush=True)
    ...
    for initialization in range(num_different_vaeac_initiate):
        ...
        for epoch in range(num_epochs_run_vaeac):
    ...
    if verbose_summary:
        print('Of the %d initiations of VAEAC, number %d was the best\n'
              'with training vlb of %.3f after %d epochs.' % (num_different_vaeac_initiate,
                                                               best_iteration,
                                                               best_train_vlb[-1],
                                                               num_epochs_run_vaeac), file=stderr, flush=True)
    ...
    # %%
    # Start the training loop
    for epoch in range(num_epochs_run_vaeac, epochs):
        if verbose:
            ...
            if any([
                ...
                if max(validation_iwae[::-1]) <= val_iwae:
                    best_state = deepcopy({
                        ...
                        'num_epochs_run_vaeac': num_epochs_run_vaeac,
                        'width': width,
                        'depth': depth,
                        'latent_dim': latent_dim,
                        'lr': lr,
...
def VAEAC_impute_values(instances_to_impute,
    ...
    # 'num_epochs_run_vaeac', 'width', 'depth',
